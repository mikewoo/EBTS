...
list1 = os.listdir(dir1)#列出目录下的所有文件和目录
for file_line in list1:
	filenum = filenum +1
	file_trace = dir1 +'/'+file_line
...
case_num3=0
dir3 = "./output/over"
list3 = os.listdir(dir3)#列出目录下的所有文件和目录
fout_int = open('log_sc.txt','a')
fout_int.write('整数上下溢出缺陷检测结果:\n')
for file_line in list3:
	filenum = filenum +1
	file_trace = dir3 +'/'+file_line
	if os.path:  # 如果是文件就进行处理
		fin = open(file_trace)
		...
		for line in fin:
			...
			if match1:
				...
				match3 = re.search(r'(?<=Command: ).+',line)
				if match3:
					case_num3 = case_num3+1
					fout_int.write('****执行用例编号：case_'+str(case_num3) +'\n' )
case_num2=0
dir2 = "./output/sc"
list2 = os.listdir(dir2)#列出目录下的所有文件和目录
fout_int = open('log_sc.txt','a')
fout_int.write('整数符号转换缺陷检测结果:\n')
for file_line in list2:
	filenum = filenum +1
	file_trace = dir2 +'/'+file_line
	if os.path:  # 如果是文件就进行处理
		fin = open(file_trace)
		for line in fin:
			match1 = re.search(r'^==\d+==',line)
			...
			#=============Integer overflow=============================================
				...
				match3 = re.search(r'(?<=Command: ).+',line)
				if match3:
					case_num2 = case_num2+1
					fout_int.write('****执行用例编号：case_'+str(case_num2) +'\n' )
